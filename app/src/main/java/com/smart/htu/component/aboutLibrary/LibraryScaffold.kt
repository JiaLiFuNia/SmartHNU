package com.smart.htu.component.aboutLibrary
...
fun LibraryScaffoldLayout(
    ...
    Layout(
        ...
        content = {
            Box(
                Modifier
                    .layoutId(LibraryLayoutContent.Name)
                    .padding(libraryPadding.namePadding)
                    .fillMaxWidth(), content = name
            )
            Box(
                Modifier
                    .layoutId(LibraryLayoutContent.Version)
                    .padding(libraryPadding.versionPadding), content = version
            )
        ...
        val versionMaxWidth =
            if (constraints.maxWidth == Constraints.Infinity) constraints.maxWidth else (constraints.maxWidth * 0.3f).toInt()
        val versionPlaceable = measurables.fastFirst { it.layoutId == LibraryLayoutContent.Version }
            .measure(constraints.copy(minWidth = 0, maxWidth = versionMaxWidth))

        val maxNameWidth =
            if (constraints.maxWidth == Constraints.Infinity) constraints.maxWidth else (constraints.maxWidth - versionPlaceable.width).coerceAtLeast(
                0
            )
        val namePlaceable = measurables.fastFirst { it.layoutId == LibraryLayoutContent.Name }
            .measure(constraints.copy(minWidth = 0, maxWidth = maxNameWidth))

        val nameYOffset =
            if (versionPlaceable.height > namePlaceable.height) (versionPlaceable.height - namePlaceable.height) / 2 else 0
        val versionYOffset =
            if (versionPlaceable.height < namePlaceable.height) (namePlaceable.height - versionPlaceable.height) / 2 else 0

        val topLineHeight =
            versionPlaceable.height.coerceAtLeast(namePlaceable.height) + libraryPadding.verticalPadding.toPx()
                .toInt()
        val authorPlaceable = measurables.fastFirst { it.layoutId == LibraryLayoutContent.Author }
            .measure(constraints.copy(minWidth = 0))

        val authorGuideline =
            topLineHeight + authorPlaceable.height + libraryPadding.verticalPadding.toPx().toInt()
        val descriptionPlaceable =
            measurables.fastFirst { it.layoutId == LibraryLayoutContent.Description }
                .measure(constraints.copy(minWidth = 0))

        val descriptionGuideline =
            authorGuideline + descriptionPlaceable.height + libraryPadding.verticalPadding.toPx()
                .toInt()
        val licensesPlaceable =
            measurables.fastFirst { it.layoutId == LibraryLayoutContent.Licenses }
                .measure(constraints.copy(minWidth = 0))
